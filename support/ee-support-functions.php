	    ...
	    if(is_array($eeArray)) {
		    ...
		    foreach ($eeArray as $eeKey => $eeValue) {
		        ...
		        if(!in_array($eeKey, $eeIgnore) AND $eeValue) {
					...
					if($eeKey == 'eeContact_email') {
						$eeContact_From = $eeValue;
					}
...
if(!function_exists('eeMailError')) {
	...
	function eeMailError($eeContact_wpError) {
		...
		$eeSFL_Output .= '<div class="error"><p>' . __('The message failed to send.', 'ee-protect-login-page') . '</p>';
		
		// $eeSFL_Output .= "<pre>" . print_r($eeContact_wpError, TRUE) . "</pre></div>";
